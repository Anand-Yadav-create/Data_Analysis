...
def country_event_heatmap(df,country):
    ...
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
    fx=new_df.pivot_table(index='Sport', columns='Year', values='Medal', aggfunc='count').fillna(0)
    return fx;
...
def select_country_data(df,country):
    athlete_df = df.drop_duplicates(subset=['Name', 'region'])
    temp_df = athlete_df[athlete_df['Sport'] == country].groupby('Age').sum()[
        ['Gold', 'Silver', 'Bronze']].reset_index()
    temp_df['Total'] = temp_df['Gold'] + temp_df['Silver'] + temp_df['Bronze']
    fr = temp_df[['Age', 'Total']]
